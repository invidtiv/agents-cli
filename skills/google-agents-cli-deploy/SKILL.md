...
description: >
  ...
  "set up CI/CD", "configure secrets", "troubleshoot a deployment",
  or needs guidance on Agent Engine, Cloud Run, or GKE deployment targets.
...
> **Do NOT run `agents-cli infra single-project` before deploying.** It is not a prerequisite. `agents-cli deploy` works on its own for all deployment targets.
...
| `--agent-identity` | Enable agent identity (Preview) | Agent Engine |
